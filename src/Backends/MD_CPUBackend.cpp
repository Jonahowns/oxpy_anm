...
void MD_CPUBackend::_first_step(llint curr_step) {
	...
	for(auto p : _particles) {
		if(_use_builtin_langevin_thermostat) {
			...
			LR_vector dv = p->force * (_dt * (number) 0.5); // To Do *eventually* add Inertia
			p->vel = p_plus + dv;
			dr = (p_plus + dv) * _dt;
		}
		else {
			p->vel += p->force * (_dt * (number) 0.5); //ToDO *eventually* add inertia
			dr = p->vel * _dt;
		}
		if(dr.norm() > 0.01) {
		...
		if(p->is_rigid_body()) {
			p->L += p->torque * (_dt * (number) 0.5); // ToDO *eventually* add inertia
...
void MD_CPUBackend::_second_step() {
	for(auto p : _particles) {
		p->vel += p->force * _dt * (number) 0.5f; // ToDO *eventually* add inertia
		if(_use_builtin_langevin_thermostat) {
			p->vel = _langevin_c1 * p->vel + _langevin_c2 * LR_vector(Utils::gaussian(), Utils::gaussian(), Utils::gaussian());
		}

		if(p->is_rigid_body()) {
			p->L += p->torque * _dt * (number) 0.5f; // To Do *eventually* add Inertia
		}
