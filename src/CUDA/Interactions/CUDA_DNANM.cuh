...
__forceinline__ __device__ void dnanm_excluded_volume(const c_number4 &r, c_number4 &F, c_number sigma, c_number rstar, c_number b, c_number rc) {
...
__forceinline__ __device__ c_number dnanm_f1(c_number r, int type, int n3, int n5) {
...
__forceinline__ __device__ c_number dnanm_f1D(c_number r, int type, int n3, int n5) {
    c_number val = (c_number) 0.f;
    int eps_index = 0;
    if(r < MD_F1_RCHIGH[type]) {
...
__forceinline__ __device__ c_number dnanm_f2(c_number r, int type) {
    c_number val = (c_number) 0.f;
...
__forceinline__ __device__ c_number dnanm_f2D(c_number r, int type) {
    c_number val = (c_number) 0.f;
    if(r < MD_F2_RCHIGH[type]) {
        if(r > MD_F2_RHIGH[type]) {
...
__forceinline__ __device__ c_number dnanm_f4(c_number t, float t0, float ts, float tc, float a, float b) {
    c_number val = (c_number) 0.f;
    t -= t0;
    if(t < 0) t = -t;
...
__forceinline__ __device__ c_number dnanm_f4_pure_harmonic(c_number t, float a, float b) {
...
__forceinline__ __device__ c_number dnanm_f4Dsin(c_number t, float t0, float ts, float tc, float a, float b) {
    c_number val = (c_number) 0.f;
    c_number tt0 = t - t0;
...
__forceinline__ __device__ c_number dnanm_f4Dsin_pure_harmonic(c_number t, float a, float b) {
    // for getting a f4t1 function with a continuous derivative that is less disruptive to the potential
    c_number val = (c_number) 0.f;
    c_number tt0 = t - b;
...
__forceinline__ __device__ c_number dnanm_f5(c_number f, int type) {
    c_number val = (c_number) 0.f;
...
__forceinline__ __device__ c_number dnanm_f5D(c_number f, int type) {
    c_number val = (c_number) 0.f;
...
template<bool qIsN3>
__device__ void dnanm_bonded_excluded_volume(c_number4 &r, c_number4 &n3pos_base, c_number4 &n3pos_back, c_number4 &n5pos_base, c_number4 &n5pos_back, c_number4 &F, c_number4 &T) {
    c_number4 Ftmp;
    // BASE-BASE
    c_number4 rcenter = r + n3pos_base - n5pos_base;
    dnanm_excluded_volume(rcenter, Ftmp, EXCL_S2, EXCL_R2, EXCL_B2, EXCL_RC2);
    c_number4 torquep1 = (qIsN3) ? _cross(n5pos_base, Ftmp) : _cross(n3pos_base, Ftmp);
    F += Ftmp;

    // n5-BASE vs. n3-BACK
    rcenter = r + n3pos_back - n5pos_base;
    dnanm_excluded_volume(rcenter, Ftmp, EXCL_S3, EXCL_R3, EXCL_B3, EXCL_RC3);
    c_number4 torquep2 = (qIsN3) ? _cross(n5pos_base, Ftmp) : _cross(n3pos_back, Ftmp);
    F += Ftmp;

    // n5-BACK vs. n3-BASE
    rcenter = r + n3pos_base - n5pos_back;
    dnanm_excluded_volume(rcenter, Ftmp, EXCL_S4, EXCL_R4, EXCL_B4, EXCL_RC4);
...
template<bool qIsN3>
__device__ void dnanm_bonded_part(c_number4 &n5pos, c_number4 &n5x, c_number4 &n5y, c_number4 &n5z, c_number4 &n3pos, c_number4 &n3x, c_number4 &n3y, c_number4 &n3z, c_number4 &F, c_number4 &T, bool grooving, bool use_oxDNA2_FENE, bool use_mbf, c_number mbf_xmax, c_number mbf_finf) {
    ...
    // EXCLUDED VOLUME
    dnanm_bonded_excluded_volume<qIsN3>(r, n3pos_base, n3pos_back, n5pos_base, n5pos_back, Ftmp, Ttmp);
    ...
    // functions
    c_number f1 = dnanm_f1(rstackmod, STCK_F1, n3type, n5type);
    c_number f4t4 = dnanm_f4(t4, STCK_THETA4_T0, STCK_THETA4_TS, STCK_THETA4_TC, STCK_THETA4_A, STCK_THETA4_B);
    c_number f4t5 = dnanm_f4(PI - t5, STCK_THETA5_T0, STCK_THETA5_TS, STCK_THETA5_TC, STCK_THETA5_A, STCK_THETA5_B);
    c_number f4t6 = dnanm_f4(t6, STCK_THETA6_T0, STCK_THETA6_TS, STCK_THETA6_TC, STCK_THETA6_A, STCK_THETA6_B);
    c_number f5phi1 = dnanm_f5(cosphi1, STCK_F5_PHI1);
    c_number f5phi2 = dnanm_f5(cosphi2, STCK_F5_PHI2);

    c_number energy = f1 * f4t4 * f4t5 * f4t6 * f5phi1 * f5phi2;

    if(energy != (c_number) 0) {
        // and their derivatives
        c_number f1D = dnanm_f1D(rstackmod, STCK_F1, n3type, n5type);
        c_number f4t4Dsin = dnanm_f4Dsin(t4, STCK_THETA4_T0, STCK_THETA4_TS, STCK_THETA4_TC, STCK_THETA4_A, STCK_THETA4_B);
        c_number f4t5Dsin = dnanm_f4Dsin(PI - t5, STCK_THETA5_T0, STCK_THETA5_TS, STCK_THETA5_TC, STCK_THETA5_A, STCK_THETA5_B);
        c_number f4t6Dsin = dnanm_f4Dsin(t6, STCK_THETA6_T0, STCK_THETA6_TS, STCK_THETA6_TC, STCK_THETA6_A, STCK_THETA6_B);
        c_number f5phi1D = dnanm_f5D(cosphi1, STCK_F5_PHI1);
        c_number f5phi2D = dnanm_f5D(cosphi2, STCK_F5_PHI2);
...
__device__ void dnanm_particle_particle_interaction(c_number4 ppos, c_number4 a1, c_number4 a2, c_number4 a3, c_number4 qpos, c_number4 b1, c_number4 b2, c_number4 b3, c_number4 &F, c_number4 &T, bool grooving, bool use_debye_huckel, bool use_oxDNA2_coaxial_stacking, LR_bonds pbonds, LR_bonds qbonds, int pind, int qind, CUDABox *box) {
    ...
    dnanm_excluded_volume(rbackbone, Ftmp, EXCL_S1, EXCL_R1, EXCL_B1, EXCL_RC1);
    c_number4 Ttmp = _cross(ppos_back, Ftmp);
    dnanm_bonded_excluded_volume<true>(r, qpos_base, qpos_back, ppos_base, ppos_back, Ftmp, Ttmp);
    ...
    if(int_type == 3 && SQR(HYDR_RCLOW) < rhydromodsqr && rhydromodsqr < SQR(HYDR_RCHIGH)) {
        ...
        c_number f1 = hb_multi * dnanm_f1(rhydromod, HYDR_F1, ptype, qtype);
        c_number f4t1 = dnanm_f4(t1, HYDR_THETA1_T0, HYDR_THETA1_TS, HYDR_THETA1_TC, HYDR_THETA1_A, HYDR_THETA1_B);
        c_number f4t2 = dnanm_f4(t2, HYDR_THETA2_T0, HYDR_THETA2_TS, HYDR_THETA2_TC, HYDR_THETA2_A, HYDR_THETA2_B);
        c_number f4t3 = dnanm_f4(t3, HYDR_THETA3_T0, HYDR_THETA3_TS, HYDR_THETA3_TC, HYDR_THETA3_A, HYDR_THETA3_B);
        c_number f4t4 = dnanm_f4(t4, HYDR_THETA4_T0, HYDR_THETA4_TS, HYDR_THETA4_TC, HYDR_THETA4_A, HYDR_THETA4_B);
        c_number f4t7 = dnanm_f4(t7, HYDR_THETA7_T0, HYDR_THETA7_TS, HYDR_THETA7_TC, HYDR_THETA7_A, HYDR_THETA7_B);
        c_number f4t8 = dnanm_f4(t8, HYDR_THETA8_T0, HYDR_THETA8_TS, HYDR_THETA8_TC, HYDR_THETA8_A, HYDR_THETA8_B);

        hb_energy = f1 * f4t1 * f4t2 * f4t3 * f4t4 * f4t7 * f4t8;

        if(hb_energy < (c_number) 0) {
            // derivatives called at the relevant arguments
            c_number f1D = hb_multi * dnanm_f1D(rhydromod, HYDR_F1, ptype, qtype);
            c_number f4t1Dsin = -dnanm_f4Dsin(t1, HYDR_THETA1_T0, HYDR_THETA1_TS, HYDR_THETA1_TC, HYDR_THETA1_A, HYDR_THETA1_B);
            c_number f4t2Dsin = -dnanm_f4Dsin(t2, HYDR_THETA2_T0, HYDR_THETA2_TS, HYDR_THETA2_TC, HYDR_THETA2_A, HYDR_THETA2_B);
            c_number f4t3Dsin = dnanm_f4Dsin(t3, HYDR_THETA3_T0, HYDR_THETA3_TS, HYDR_THETA3_TC, HYDR_THETA3_A, HYDR_THETA3_B);
            c_number f4t4Dsin = dnanm_f4Dsin(t4, HYDR_THETA4_T0, HYDR_THETA4_TS, HYDR_THETA4_TC, HYDR_THETA4_A, HYDR_THETA4_B);
            c_number f4t7Dsin = -dnanm_f4Dsin(t7, HYDR_THETA7_T0, HYDR_THETA7_TS, HYDR_THETA7_TC, HYDR_THETA7_A, HYDR_THETA7_B);
            c_number f4t8Dsin = dnanm_f4Dsin(t8, HYDR_THETA8_T0, HYDR_THETA8_TS, HYDR_THETA8_TC, HYDR_THETA8_A, HYDR_THETA8_B);
    ...
    if(SQR(CRST_RCLOW) < rcstackmodsqr && rcstackmodsqr < SQR(CRST_RCHIGH)) {
        ...
        // functions called at their relevant arguments
        c_number f2 = dnanm_f2(rcstackmod, CRST_F2);
        c_number f4t1 = dnanm_f4(t1, CRST_THETA1_T0, CRST_THETA1_TS, CRST_THETA1_TC, CRST_THETA1_A, CRST_THETA1_B);
        c_number f4t2 = dnanm_f4(t2, CRST_THETA2_T0, CRST_THETA2_TS, CRST_THETA2_TC, CRST_THETA2_A, CRST_THETA2_B);
        c_number f4t3 = dnanm_f4(t3, CRST_THETA3_T0, CRST_THETA3_TS, CRST_THETA3_TC, CRST_THETA3_A, CRST_THETA3_B);
        c_number f4t4 = dnanm_f4(t4, CRST_THETA4_T0, CRST_THETA4_TS, CRST_THETA4_TC, CRST_THETA4_A, CRST_THETA4_B) + dnanm_f4(PI - t4, CRST_THETA4_T0, CRST_THETA4_TS, CRST_THETA4_TC, CRST_THETA4_A, CRST_THETA4_B);
        c_number f4t7 = dnanm_f4(t7, CRST_THETA7_T0, CRST_THETA7_TS, CRST_THETA7_TC, CRST_THETA7_A, CRST_THETA7_B) + dnanm_f4(PI - t7, CRST_THETA7_T0, CRST_THETA7_TS, CRST_THETA7_TC, CRST_THETA7_A, CRST_THETA7_B);
        c_number f4t8 = dnanm_f4(t8, CRST_THETA8_T0, CRST_THETA8_TS, CRST_THETA8_TC, CRST_THETA8_A, CRST_THETA8_B) + dnanm_f4(PI - t8, CRST_THETA8_T0, CRST_THETA8_TS, CRST_THETA8_TC, CRST_THETA8_A, CRST_THETA8_B);

        c_number cstk_energy = f2 * f4t1 * f4t2 * f4t3 * f4t4 * f4t7 * f4t8;

        if(cstk_energy < (c_number) 0) {
            // derivatives called at the relevant arguments
            c_number f2D = dnanm_f2D(rcstackmod, CRST_F2);
            c_number f4t1Dsin = -dnanm_f4Dsin(t1, CRST_THETA1_T0, CRST_THETA1_TS, CRST_THETA1_TC, CRST_THETA1_A, CRST_THETA1_B);
            c_number f4t2Dsin = -dnanm_f4Dsin(t2, CRST_THETA2_T0, CRST_THETA2_TS, CRST_THETA2_TC, CRST_THETA2_A, CRST_THETA2_B);
            c_number f4t3Dsin = dnanm_f4Dsin(t3, CRST_THETA3_T0, CRST_THETA3_TS, CRST_THETA3_TC, CRST_THETA3_A, CRST_THETA3_B);
            c_number f4t4Dsin = dnanm_f4Dsin(t4, CRST_THETA4_T0, CRST_THETA4_TS, CRST_THETA4_TC, CRST_THETA4_A, CRST_THETA4_B) - dnanm_f4Dsin(PI - t4, CRST_THETA4_T0, CRST_THETA4_TS, CRST_THETA4_TC, CRST_THETA4_A, CRST_THETA4_B);
            c_number f4t7Dsin = -dnanm_f4Dsin(t7, CRST_THETA7_T0, CRST_THETA7_TS, CRST_THETA7_TC, CRST_THETA7_A, CRST_THETA7_B) + dnanm_f4Dsin(PI - t7, CRST_THETA7_T0, CRST_THETA7_TS, CRST_THETA7_TC, CRST_THETA7_A, CRST_THETA7_B);
            c_number f4t8Dsin = dnanm_f4Dsin(t8, CRST_THETA8_T0, CRST_THETA8_TS, CRST_THETA8_TC, CRST_THETA8_A, CRST_THETA8_B) - dnanm_f4Dsin(PI - t8, CRST_THETA8_T0, CRST_THETA8_TS, CRST_THETA8_TC, CRST_THETA8_A, CRST_THETA8_B);
    ...
    if(use_oxDNA2_coaxial_stacking) {
        ...
        if(SQR(CXST_RCLOW) < rstackmodsqr && rstackmodsqr < SQR(CXST_RCHIGH)) {
            ...
            // functions called at their relevant arguments
            c_number f2 = dnanm_f2(rstackmod, CXST_F2);
            c_number f4t1 = dnanm_f4(t1, CXST_THETA1_T0_OXDNA2, CXST_THETA1_TS, CXST_THETA1_TC, CXST_THETA1_A, CXST_THETA1_B) + dnanm_f4_pure_harmonic(t1, CXST_THETA1_SA, CXST_THETA1_SB);
            c_number f4t4 = dnanm_f4(t4, CXST_THETA4_T0, CXST_THETA4_TS, CXST_THETA4_TC, CXST_THETA4_A, CXST_THETA4_B);
            c_number f4t5 = dnanm_f4(t5, CXST_THETA5_T0, CXST_THETA5_TS, CXST_THETA5_TC, CXST_THETA5_A, CXST_THETA5_B) + dnanm_f4(PI - t5, CXST_THETA5_T0, CXST_THETA5_TS, CXST_THETA5_TC, CXST_THETA5_A, CXST_THETA5_B);
            c_number f4t6 = dnanm_f4(t6, CXST_THETA6_T0, CXST_THETA6_TS, CXST_THETA6_TC, CXST_THETA6_A, CXST_THETA6_B) + dnanm_f4(PI - t6, CXST_THETA6_T0, CXST_THETA6_TS, CXST_THETA6_TC, CXST_THETA6_A, CXST_THETA6_B);

            c_number cxst_energy = f2 * f4t1 * f4t4 * f4t5 * f4t6;

            if(cxst_energy < (c_number) 0) {
                // derivatives called at the relevant arguments
                c_number f2D = dnanm_f2D(rstackmod, CXST_F2);
                c_number f4t1Dsin = -dnanm_f4Dsin(t1, CXST_THETA1_T0_OXDNA2, CXST_THETA1_TS, CXST_THETA1_TC, CXST_THETA1_A, CXST_THETA1_B) - dnanm_f4Dsin_pure_harmonic(t1, CXST_THETA1_SA, CXST_THETA1_SB);
                c_number f4t4Dsin = dnanm_f4Dsin(t4, CXST_THETA4_T0, CXST_THETA4_TS, CXST_THETA4_TC, CXST_THETA4_A, CXST_THETA4_B);
                c_number f4t5Dsin = dnanm_f4Dsin(t5, CXST_THETA5_T0, CXST_THETA5_TS, CXST_THETA5_TC, CXST_THETA5_A, CXST_THETA5_B) - dnanm_f4Dsin(PI - t5, CXST_THETA5_T0, CXST_THETA5_TS, CXST_THETA5_TC, CXST_THETA5_A, CXST_THETA5_B);
                c_number f4t6Dsin = -dnanm_f4Dsin(t6, CXST_THETA6_T0, CXST_THETA6_TS, CXST_THETA6_TC, CXST_THETA6_A, CXST_THETA6_B) + dnanm_f4Dsin(PI - t6, CXST_THETA6_T0, CXST_THETA6_TS, CXST_THETA6_TC, CXST_THETA6_A, CXST_THETA6_B);
        ...
        if(SQR(CXST_RCLOW) < rstackmodsqr && rstackmodsqr < SQR(CXST_RCHIGH)) {
            ...
            // functions called at their relevant arguments
            c_number f2 = dnanm_f2(rstackmod, CXST_F2);
            c_number f4t1 = dnanm_f4(t1, CXST_THETA1_T0_OXDNA, CXST_THETA1_TS, CXST_THETA1_TC, CXST_THETA1_A, CXST_THETA1_B) + dnanm_f4(2 * PI - t1, CXST_THETA1_T0_OXDNA, CXST_THETA1_TS, CXST_THETA1_TC, CXST_THETA1_A, CXST_THETA1_B);
            c_number f4t4 = dnanm_f4(t4, CXST_THETA4_T0, CXST_THETA4_TS, CXST_THETA4_TC, CXST_THETA4_A, CXST_THETA4_B);
            c_number f4t5 = dnanm_f4(t5, CXST_THETA5_T0, CXST_THETA5_TS, CXST_THETA5_TC, CXST_THETA5_A, CXST_THETA5_B) + dnanm_f4(PI - t5, CXST_THETA5_T0, CXST_THETA5_TS, CXST_THETA5_TC, CXST_THETA5_A, CXST_THETA5_B);
            c_number f4t6 = dnanm_f4(t6, CXST_THETA6_T0, CXST_THETA6_TS, CXST_THETA6_TC, CXST_THETA6_A, CXST_THETA6_B) + dnanm_f4(PI - t6, CXST_THETA6_T0, CXST_THETA6_TS, CXST_THETA6_TC, CXST_THETA6_A, CXST_THETA6_B);
            c_number f5cosphi3 = dnanm_f5(cosphi3, CXST_F5_PHI3);
            ...
            if(cxst_energy < (c_number) 0) {
                // derivatives called at the relevant arguments
                c_number f2D = dnanm_f2D(rstackmod, CXST_F2);
                c_number f4t1Dsin = -dnanm_f4Dsin(t1, CXST_THETA1_T0_OXDNA, CXST_THETA1_TS, CXST_THETA1_TC, CXST_THETA1_A, CXST_THETA1_B) + dnanm_f4Dsin(2 * PI - t1, CXST_THETA1_T0_OXDNA, CXST_THETA1_TS, CXST_THETA1_TC, CXST_THETA1_A, CXST_THETA1_B);
                c_number f4t4Dsin = dnanm_f4Dsin(t4, CXST_THETA4_T0, CXST_THETA4_TS, CXST_THETA4_TC, CXST_THETA4_A, CXST_THETA4_B);
                c_number f4t5Dsin = dnanm_f4Dsin(t5, CXST_THETA5_T0, CXST_THETA5_TS, CXST_THETA5_TC, CXST_THETA5_A, CXST_THETA5_B) - dnanm_f4Dsin(PI - t5, CXST_THETA5_T0, CXST_THETA5_TS, CXST_THETA5_TC, CXST_THETA5_A, CXST_THETA5_B);
                c_number f4t6Dsin = -dnanm_f4Dsin(t6, CXST_THETA6_T0, CXST_THETA6_TS, CXST_THETA6_TC, CXST_THETA6_A, CXST_THETA6_B) + dnanm_f4Dsin(PI - t6, CXST_THETA6_T0, CXST_THETA6_TS, CXST_THETA6_TC, CXST_THETA6_A, CXST_THETA6_B);
                c_number f5cosphi3D = dnanm_f5D(cosphi3, CXST_F5_PHI3);
...
//
//           dnanm_particle_particle_interaction(ppos, a1, a2, a3, qpos, b1, b2, b3, F, T, grooving, use_debye_huckel, use_oxDNA2_coaxial_stacking, bs, qbonds, IND, j, box);
//        }
//    }
//
...
//   dnanm_particle_particle_interaction(ppos, a1, a2, a3, qpos, b1, b2, b3, dF, dT, grooving, use_debye_huckel, use_oxDNA2_coaxial_stacking, pbonds, qbonds, b.from, b.to, box);
//
...
//       dnanm_particle_particle_interaction(ppos, a1, a2, a3, qpos, b1, b2, b3, F, T, grooving, use_debye_huckel, use_oxDNA2_coaxial_stacking, pbonds, qbonds, IND, k_index, box);
//    }
//
//    T = _vectors_transpose_c_number4_product(a1, a2, a3, T);
//
...
//
//__global__ void distOpPrecalc(c_number4 *poss, GPU_quat *orientations, int *op_pairs1, int *op_pairs2, c_number *op_dists, int n_threads, CUDABox *box) {
//    if(IND >= n_threads) return;
//
//    int pind = op_pairs1[IND];
//    int qind = op_pairs2[IND];
...
__global__ void dnanm_forces_edge_nonbonded_angular(c_number4 *poss, GPU_quat *orientations, c_number4 *forces, c_number4 *torques, edge_bond *edge_list, int n_edges, LR_bonds *bonds, bool grooving, bool use_debye_huckel, bool use_oxDNA2_coaxial_stacking, CUDABox *box) {
    ...
    c_number4 dF = make_c_number4(0, 0, 0, 0);
    c_number4 dT = make_c_number4(0, 0, 0, 0);
    c_number4 proT = make_c_number4(0, 0, 0, 0);
    ...
    if (pbtype > 4 && qbtype > 4) {
        ...
        dT += _cross(ppos_back, Ftmp); // cross of point of contact on protein sphere and dna backbone
        proT = _cross(_normalize(rback)*(MD_pro_sigma/2), -Ftmp);
        dF += Ftmp;
        ...
        proT += _cross(_normalize(rbase)*(MD_pro_sigma/2), -Ftmp); // cross of point of contact on protein sphere and dna base
        dF += Ftmp;
        ...
        if ((dF.x * dF.x + dF.y * dF.y + dF.z * dF.z + dF.w * dF.w) > (c_number) 0.f)
            LR_atomicAddXYZ(&(forces[to_index]), dF);
        if ((dT.x * dT.x + dT.y * dT.y + dT.z * dT.z + dT.w * dT.w) > (c_number) 0.f)
            LR_atomicAddXYZ(&(torques[to_index]), proT);
        ...
        c_number4 rback = -1*(r - qpos_back);
        c_number4 rbase = -1*(r - qpos_base);

        c_number4 Ftmp = make_c_number4(0, 0, 0, 0);
        excluded_volume_quart(rback, Ftmp, MD_pro_backbone_sigma, MD_pro_backbone_rstar, MD_pro_backbone_b, MD_pro_backbone_rc);
        dT += _cross(qpos_back, Ftmp);
        proT = _cross(_normalize(rback)*(MD_pro_sigma/2), -Ftmp); // cross of point of contact b/t protein sphere and dna backbone and excluded volume force
        dF += Ftmp;

        excluded_volume_quart(rbase, Ftmp, MD_pro_base_sigma, MD_pro_base_rstar, MD_pro_base_b, MD_pro_base_rc);
        dT += _cross(qpos_base, Ftmp);
        proT += _cross(_normalize(rbase)*(MD_pro_sigma/2), -Ftmp); // cross of point of contact b/t protein sphere and dna base and excluded volume force
        dF += Ftmp;
        ...
        //add forces and torque to p index
        if ((dF.x * dF.x + dF.y * dF.y + dF.z * dF.z + dF.w * dF.w) > (c_number) 0.f)
            LR_atomicAddXYZ(&(forces[from_index]), dF);
        if ((dT.x * dT.x + dT.y * dT.y + dT.z * dT.z + dT.w * dT.w) > (c_number) 0.f)
            LR_atomicAddXYZ(&(torques[from_index]), proT);
...
__global__ void dnanm_forces_edge_nonbonded(c_number4 *poss, GPU_quat *orientations, c_number4 *forces, c_number4 *torques, edge_bond *edge_list, int n_edges, LR_bonds *bonds, bool grooving, bool use_debye_huckel, bool use_oxDNA2_coaxial_stacking, CUDABox *box) {
    if (IND >= n_edges) return;
    ...
    if (pbtype > 4 && qbtype > 4) {
    ...
    } else if (pbtype < 4 && qbtype > 4) { // p is dna q is pro
        //Protein-DNA Excluded Volume **NONBONDED
        c_number4 ppos_back;
        if(grooving) ppos_back = POS_MM_BACK1 * a1 + POS_MM_BACK2 * a2;
        else ppos_back = POS_BACK * a1;
        ...
        //vector r is reversed from the above case
        c_number4 rback = -1*(r - qpos_back); // flipped to have it pointing from nucleotide backbone to protein
        c_number4 rbase = -1*(r - qpos_base); // flipped to have it pointing from nucleotide base to protein
...
//    T0.w = torques[IND].w;
    ...
    if (pbtype < 4){
        ...
        if(bs.n3 != P_INVALID) {
            ...
            dnanm_bonded_part<true>(ppos, a1, a2, a3, qpos, b1, b2, b3, dF, dT, grooving, use_oxDNA2_FENE, use_mbf, mbf_xmax, mbf_finf);
        ...
        if(bs.n5 != P_INVALID) {
            ...
            dnanm_bonded_part<false>(qpos, b1, b2, b3, ppos, a1, a2, a3, dF, dT, grooving, use_oxDNA2_FENE, use_mbf, mbf_xmax, mbf_finf);
