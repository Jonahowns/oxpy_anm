...
__global__ void brownian_thermostat(curandState *rand_state, c_number *masses, c_number4 *vels, c_number4 *Ls, c_number rescale_factor, c_number pt, c_number pr, int N) {
	if(IND < N) {
		...
		if(curand_uniform(&state) < pt) {
			...
			c_number mass_rescale_factor = rescale_factor/sqrtf(masses[IND]);
...
void CUDABrownianThermostat::apply_cuda(c_number *d_masses, c_number4 *d_poss, GPU_quat *d_orientations, c_number4 *d_vels, c_number4 *d_Ls, llint curr_step) {
	if(!would_activate(curr_step)) return;

	brownian_thermostat
		<<<this->_launch_cfg.blocks, this->_launch_cfg.threads_per_block>>>
		(this->_d_rand_state, d_masses, d_vels, d_Ls, this->_rescale_factor, this->_pt, this->_pr, CONFIG_INFO->N());
}
