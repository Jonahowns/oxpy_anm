...
__global__ void langevin_thermostat(curandState *rand_state, c_number *massesInv, c_number4 *vels, c_number4 *Ls, c_number _dt, c_number gamma_trans, c_number rescale_factor_trans, c_number gamma_rot, c_number rescale_factor_rot, int N) {
	if(IND < N) {
		...
		v.x += _dt * (-gamma_trans * v.x + f_vx * rescale_factor_trans * sqrtf(massesInv[IND]));
		v.y += _dt * (-gamma_trans * v.y + f_vy * rescale_factor_trans * sqrtf(massesInv[IND]));
		v.z += _dt * (-gamma_trans * v.z + f_vz * rescale_factor_trans * sqrtf(massesInv[IND]));
...
void CUDALangevinThermostat::apply_cuda(c_number *d_mass, c_number4 *d_poss, GPU_quat *d_orientations, c_number4 *d_vels, c_number4 *d_Ls, llint curr_step) {
	if(!would_activate(curr_step)) return;

	langevin_thermostat
		<<<this->_launch_cfg.blocks, this->_launch_cfg.threads_per_block>>>
		(this->_d_rand_state, d_mass, d_vels, d_Ls, this->_dt, this->_gamma_trans, this-> _rescale_factor_trans, this->_gamma_rot, this->_rescale_factor_rot, CONFIG_INFO->N());
}
