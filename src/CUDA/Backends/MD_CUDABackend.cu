	...
	if(_external_forces) {
		...
		for(int i = 0; i < N(); i++) {
			...
			for(uint j = 0; j < p->ext_forces.size(); j++) {
                ...
                else if(force_type == typeid(SkewTrap)) {
					SkewTrap *p_force = (SkewTrap *) p->ext_forces[j];
					init_SkewTrap_from_CPU(&cuda_force->ltcomtrap, p_force, first_time);
				}
                else if(force_type == typeid(Morse)) {
...
void MD_CUDABackend::_apply_barostat() {
	...
	if(_cuda_barostat_always_refresh) {
		...
		_cuda_barostat_thermostat->apply_cuda(_d_massInv, _d_poss, _d_orientations, _d_vels, _d_Ls, current_step());
	}
}
...
void MD_CUDABackend::_thermalize() {
	_cuda_thermostat->apply_cuda(_d_massInv, _d_poss, _d_orientations, _d_vels, _d_Ls, current_step());
}
