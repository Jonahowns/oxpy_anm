...
def get_top_info(top : str) -> TopInfo:
    ...
    with open(top) as f:
        ...
        if len(my_top_info)  == 2:
            ...
            print("ERROR: malformed topology header, failed to read topology file. Length of top_info = "+str(len(my_top_info)), file=stderr)
            sys.exit("ERROR: malformed topology header, failed to read topology file. Length of top_info = "+str(len(my_top_info)))
